...
const server = net.createServer((socket) => {
  ...
  socket.on('data', (data) => {
    ...
    if (!handler) {
      console.error(`Error: No handler found for ID ${length}`);
      socket.write(`Error: Invalid hanler ID ${handlerId}`);
      socket.end();
      return;
    }
...
server.listen(process.env.PORT, () => {
  console.log(`Echo Server listening on prot ${process.env.PORT}`);
